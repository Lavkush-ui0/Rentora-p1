...
import { TrendingUp, Zap, DollarSign, Star, ChevronRight, ArrowRight } from 'lucide-react';
...
export const Home: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await discoveryService.getHomepageData();
        if (res.data?.success) {
  ...
  const SectionHeader = ({ icon: Icon, title, subtitle, linkTo }: any) => (
    <div className="flex items-center justify-between mb-6">
      <div className="flex items-center space-x-3">
        <div className="p-2 rounded-2xl bg-primary-50 dark:bg-primary-950/30">
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Hero Welcome Banner */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-primary-600 via-primary-700 to-indigo-600 p-8 md:p-10 text-white shadow-2xl shadow-primary-500/25">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 right-0 w-64 h-64 bg-white rounded-full -translate-y-1/2 translate-x-1/2"></div>
          <div className="absolute bottom-0 left-0 w-48 h-48 bg-white rounded-full translate-y-1/2 -translate-x-1/2"></div>
        </div>
        <div className="relative z-10">
          <div className="inline-flex items-center space-x-2 bg-white/20 px-3 py-1.5 rounded-full text-xs font-bold mb-4">
            <span className="h-2 w-2 bg-green-400 rounded-full animate-pulse"></span>
            <span>NIET Exclusive Marketplace</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-black font-outfit leading-tight mb-3">
            Rent Anything.<br/>
            <span className="text-primary-200">From Your Classmates.</span>
          </h1>
          <p className="text-primary-100 text-sm md:text-base max-w-md mb-6 leading-relaxed">
            Books, calculators, lab coats, gaming controllers — borrow what you need and earn by sharing what you have.
          </p>
          <div className="flex flex-wrap gap-3">
            <Link
              to="/explore"
              className="bg-white text-primary-700 font-bold px-5 py-2.5 rounded-full text-sm hover:bg-primary-50 transition-all flex items-center space-x-1.5"
            >
      ...
      <div className="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-2xl text-center">
        <p className="text-sm text-amber-800 dark:text-amber-400 font-medium">
          💸 <strong>Payment Notice:</strong> All payments are handled <strong>offline between students</strong>. Rentora does not process or hold any money.
        </p>
      </div>
              ...
              >
                <div className="relative">
                  ...
                  <div className="absolute -bottom-1 -right-1 bg-amber-400 text-amber-900 text-[9px] font-extrabold px-1.5 py-0.5 rounded-full border border-white dark:border-slate-900 shadow">
                    ⭐ {student.ratingAverage.toFixed(1)}
                  </div>
                </div>
                <div>
